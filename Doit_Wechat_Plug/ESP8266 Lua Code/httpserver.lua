...
local function sendParaData(conn)
	local para1="'stassid':'"..cfgSTA.ssid.."',".."'stapsw':'"..cfgSTA.psw.."',"
	local para2="'stamac':'"..wifi.sta.getmac().."',".."'apmac':'"..wifi.ap.getmac().."'"
	local para3="settingsCallback({"..para1..para2.."})"
	local para =[[HTTP/1.1 200 OK
Content-Type:text/html
Content-Length: ]]..string.len(para3).."\r\n" .. [[	
Connection: close

]]
	conn:send(para..para3);
	para=nil;para1=nil;para2=nil;para3=nil
	collectgarbage()
end

local function parsePara(request)
local k,_ = string.find(request, "stassid=");
...
Connection: close
		...
		elseif string.find(request, "GET /para.js HTTP/1.1") then
			sendParaData(conn)
