...
/*
  ...
  因此，串口0不可使用，程序使用Serial.swap()将串口0从映射到gpio15和gpio13,（gpio13被PinDirB占用）
  因此开发板方口usb无输出，要查看串口输出，使用usb-ttl在gpio
*/
...
void setup() {
  initParseData();
  Serial.begin (115200);
  Serial.swap();//change tx0/rx0 to  GPIO15 (TX) and GPIO13 (RX)
  Serial.println("\r\n\r\nStart ESP8266");
