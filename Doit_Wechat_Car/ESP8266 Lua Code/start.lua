...
5,  若没有配置sta，则启动的时候不连接sta。否则连接，尝试60秒仍然没有成功的话，停止连接
]]
...
function startAPSTA()
	...
	cfgTemp=nil
	if(string.len(cfgSTA.psw)>1 and string.len(cfgSTA.psw)<8) then
		--print("Password: 0,8~64")
		cfgSTA.psw='';
	end
	if cfgSTA.ssid ~='' then
...
function startSmartConfig(mode)
	...
	wifi.startsmart(mode, function(ssid, psw)
		...
		tmr.alarm(0,5000,1,function() node.restart() end)
	end)
		ledCnt=0;ledCnt2=0;
...
print('Module Start')
print('Short press key: esptouch mode / long press key: airkiss mode')
...
if cfgSTA.ssid ==nil or cfgSTA.psw == nil then 
	resetCfgSTA()
	dofile('configSTA.lua')
end
gpioInit()
...
tmr.alarm(1,100,1,function()
	...
	if tryCnt == 600 then
		print('STA Timer out stop connect');
		tmr.stop(1);
		wifi.sta.disconnect()
	end
  end)
